...
import { codes, invalid, name, valid, validOnlyByFormat } from './unitedKingdom.vat';
...
describe('United Kingdom', () => {
  ...
  it('should return "true" for "isSupportedCountry" and "isValidFormat" fields, but "false" for "isValid" for VATs that match format but still invalid', () => {
    validOnlyByFormat.forEach((vat) => checkOnlyValidFormatVat(vat, [unitedKingdom], codes, name));
  });

  it('should return "true" result for valid VATs with extra dash characters', () => {
    valid.map((vat) => addCharsToString(vat, '-')).forEach((vat) => checkValidVat(vat, [unitedKingdom], codes, name));
  });
